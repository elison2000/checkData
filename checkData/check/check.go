...
func CreateParallelPool(parallel int, todoQue, doneQue chan Checker) {
	...
	for i := 1; i < parallel+1; i++ {
		wg.Add(1)
		go func(rid int, wg *sync.WaitGroup, todoQue, doneQue chan Checker) {
			defer wg.Done()
			for todo := range todoQue {
				slog.Infof("Routine%d开始执行任务 [%s]", rid, todo.GetName())
				CheckTable(todo) //这里的todo必须是指针类型*MysqlTable
				doneQue <- todo
			}
		}(i, &wg, todoQue, doneQue)
	}
	//等待所有任务执行完毕
	wg.Wait()
}
...
func CheckMysqlDB(opt util.Options, dbgroup [2]string) {
	...
	//开始核对
	slog.Infof("[%s:%s] 开始核对数据库 [SOURCE端表数:%d  TARGET端表数:%d  需要核对的表数:%d]", dbgroup[0], dbgroup[1], len(db.SourceTables), len(db.TargetTables), len(db.ToCheckTables))
...
func CheckMongoDB(opt util.Options, dbgroup [2]string) {
	...
	//开始核对
	slog.Infof("[%s:%s] 开始核对数据库 [SOURCE端表数:%d  TARGET端表数:%d  需要核对表数:%d]", dbgroup[0], dbgroup[1], len(db.SourceTables), len(db.TargetTables), len(db.ToCheckTables))
