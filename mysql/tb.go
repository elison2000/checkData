...
func (self *Table) getTableColumns() error {
    ...
    cols := []string{}
    skipcols := []string{}
    sql := fmt.Sprintf("desc `%s`", self.TbName)
    rows, err := self.DbGroup.SourceDbConn.QueryReturnList(sql)
    if err != nil {
        return fmt.Errorf("getTableColumns -> %w", err)
    }
...
func (self *Table) getCheckSql() error {
    ...
    if len(self.Keys) == 0 {
        return fmt.Errorf("GetCRC32DataSQL -> 获取主键失败")
    }
    if len(self.Columns) == 0 {
        return fmt.Errorf("GetCRC32DataSQL -> 获取列名失败")
    }
...
func (self *Table) GetRepairSQL(idText string, mode int) (string, error) {
    ...
    case 0, 1:
        ...
        for _, v := range rows {
            if mode == 1 {
                sqlText.WriteString(fmt.Sprintf("INSERT INTO `%s` (%s) VALUES %s;\n", self.TbName, ColumnsText, v))
            } else {
                sqlText.WriteString(fmt.Sprintf("REPLACE INTO `%s` (%s) VALUES %s;\n", self.TbName, ColumnsText, v))
            }
        }
    }
