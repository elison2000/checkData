...
func (this *MysqlDb) Init() {
	//获取数据库连接
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?timeout=5s", this.User, this.Password, this.Host, this.Port, this.Database)
...
func (this *MysqlDb) QueryReturnList(sqltext string) ([][]string, error) {
	//执行sql，返回二维数组
...
func (this *MysqlDb) QueryReturnText(sqltext string) ([]string, error) {
	//执行sql，返回二维数组
	// 查询数据
	cur, err := this.ConnPool.Query(sqltext)
	if err != nil {
...
func (this *MysqlDb) QueryReturnDict(sqltext string) ([]map[string]string, error) {
	//执行sql，返回二维map
	cur, err := this.ConnPool.Query(sqltext)
	if err != nil {
		return nil, err
