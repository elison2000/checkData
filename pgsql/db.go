...
func NewDatabase(opt *model.Options, dbg [2]string) (*Database, error) {
    ...
    db := Database{
        ...
        SourceDbConn: &util.PgsqlDb{Host: opt.SourceHost, Port: opt.SourcePort, User: opt.User, Password: opt.Password, Database: dbg[0]},
        TargetDbConn: &util.PgsqlDb{Host: opt.TargetHost, Port: opt.TargetPort, User: opt.TargetUser, Password: opt.TargetPassword, Database: dbg[1]},
        Tables:       &model.Tables{},
    }
