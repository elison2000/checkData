...
type PgsqlDatabase struct {
	...
	TargetHost       string
	TargetPort       int
	SourceDbConn     *util.PgsqlDb
	TargetDbConn     *util.PgsqlDb
	SourceTables     []string
	TargetTables     []string
...
func (this *PgsqlDatabase) Init(opt util.Options, dbgroup [2]string) {
	...
	this.SourceDbConn = &util.PgsqlDb{Host: this.SourceHost, Port: this.SourcePort, User: opt.User, Password: opt.Password, Database: this.SourceDb}
	this.TargetDbConn = &util.PgsqlDb{Host: this.TargetHost, Port: this.TargetPort, User: opt.User, Password: opt.Password, Database: this.TargetDb}
...
func (this *PgsqlDatabase) GetTables() {
	// 获取表名
	sql := `select concat(schemaname,'.',tablename) from pg_tables where schemaname not in ('pg_catalog','pgpool_catalog','information_schema')`
	//获取源库所有表
...
func (this *PgsqlDatabase) GetToCheckTables() {
	//获取两端都存在的表
...
type PgsqlTable struct {
	...
	RecheckPassRows int
	ExecuteSeconds  int
	DbGroup         *PgsqlDatabase
}

func NewPgsqlTable(tb string, dbg *PgsqlDatabase, opt util.Options) *PgsqlTable {
	//MysqlTable构造函数
...
func (this *PgsqlTable) SplitTableName() (string, string) {
	//拆分列名
	l := strings.Split(this.TbName, `.`)
	schema := l[0]
	tb := l[1]
	return schema, tb
}

func (this *PgsqlTable) GetTableColumns() {
	// 获取表列名
	defer func() {
...
order by array_position(pg_index.indkey, pg_attribute.attnum)`, schema, tb)
	...
	//获取列名
	rows1, err1 := this.DbGroup.SourceDbConn.QueryReturnList(sql1)
	if err1 != nil {
		panic(err1)
	}
	...
	//获取主键
	if len(this.Keys) == 0 {
		rows2, err2 := this.DbGroup.SourceDbConn.QueryReturnList(sql2)
		if err2 != nil {
			panic(err2)
		}
...
func (this *PgsqlTable) GetCRC32DataSQL() {
	//生成计算CRC32数据的SQL
	var sql string
...
func (this *PgsqlTable) GetMode() string {
	return this.Mode
}

func (this *PgsqlTable) Precheck() bool {
	//预检查
	...
	if len(this.Columns) > 99 {
		this.Result = "unknown"
		this.Error = "表的列数不能超过99"
		slog.Errorf("[%s.%s] 表的列数不能超过99", this.DbName, this.TbName)
		return false
	}
	if this.Error == "" {
...
func (this *PgsqlTable) GetSourceCRC32Data(stop chan struct{}) {
...
func (this *PgsqlTable) GetTargetCRC32Data(stop chan struct{}) {
	...
	rows, err := this.DbGroup.TargetDbConn.ConnPool.Query(this.SqlText)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		err := rows.Scan(&data.Id, &data.Sum)
		if err != nil {
			panic(err)
		}
		select {
...
func (this *PgsqlTable) GetSourceCRC32DataSlow(stop chan struct{}) {
	...
	slog.Infof("[%s.%s] 开始下载source端数据", this.DbGroup.SourceDb, this.TbName)
	rows, err := this.DbGroup.SourceDbConn.ConnPool.Query(this.SqlText)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
...
func (this *PgsqlTable) GetTargetCRC32DataSlow(stop chan struct{}) {
...
func (this *PgsqlTable) GetSourceTableCount(wg *sync.WaitGroup) {
	// 返回表总行数
...
func (this *PgsqlTable) GetTargetTableCount(wg *sync.WaitGroup) {
	// 返回表总行数
	defer util.TimeCost()(fmt.Sprintf("[%s.%s] Target端总行数统计完成", this.DbGroup.TargetDb, this.TbName))
	//defer this.TargetDbPool.Close()
	defer (*wg).Done()
	defer func() {
		if msg := recover(); msg != nil {
			this.Result = "unknown"
...
func (this *PgsqlTable) GetRepairSQL(idtext string, mode int) (string, error) {
	// 生成replace sql
	// mode:修复模式, 0-delete 1-insert 2-update
	...
	if mode == 0 {
		...
		for _, rowtext := range rows {
			//拼接insert SQL
			sqltext += fmt.Sprintf("INSERT INTO %s (%s) VALUES %s;\n", this.TbName, ColumnsText, rowtext)
		}
	}
	return sqltext, nil
}

func (this *PgsqlTable) CheckTableCount() {
	// 检查表总行数
...
func (this *PgsqlTable) CheckTableDetail() {
	// 开始核对行明细
	...
	if this.Mode == "slow" {
		go this.GetSourceCRC32DataSlow(stopChan)
		go this.GetTargetCRC32DataSlow(stopChan)
	} else {
		go this.GetSourceCRC32Data(stopChan)
		go this.GetTargetCRC32Data(stopChan)
	}
...
func (this *PgsqlTable) RecheckOneRow(idtext string) bool {
	//复核一行数据,相同返回true
	var sql, where string
	ColumnsText := strings.Join(util.EncloseStringArray(this.Columns, `"`), ", ")
...
func (this *PgsqlTable) RecheckRows(idtextListP *[]string, recheckPassListP *[]string) {
	//复核一个数组的idtext
	for i := 1; i <= 3; i++ {
...
func (this *PgsqlTable) GetResults() util.CheckResults {
	//返回核对结果
